...
2. **`prompt.py`** — Prompt templates: `JOB_EXTRACTION_PROMPT` (with granularity rules: umbrella extraction for grouped items, composite practice detection, role-as-skill prevention, dedup, specificity preservation, responsibility-vs-skill boundary with YES/NO examples), `CV_EXTRACTION_PROMPT`, `SKILL_EVAL_PROMPT` (placeholders: `{skill}`, `{context}`, `{rules}`), `SKILL_CLASSIFIER_PROMPT`, `WRITER_PROMPT` (placeholders: `{title}`, `{responsibilities}`, `{analysis}`, `{cv_text}` — analysis includes per-skill evidence inline; rules enforce matched skills as single source of truth, block trivial rephrasing suggestions), and `SCORER_PROMPT` (placeholders: `{job_profile}`, `{analysis}`, `{cover_letter}`, `{candidate_experience}` — three weighted dimensions: Skill Match 35%, Experience Relevance 35%, Overall Presentation 30%). Scorer has strict dimension boundaries: Experience Relevance must NOT reference individual skills by name (only assesses domain fit, level fit, breadth), respects JDs that explicitly accept project/internship experience. Three rule blocks injected per skill type: `HARD_SKILL_EVAL_RULES` (explicit mention, direct technical synonym, or any one slash-separated alternative), `SOFT_SKILL_EVAL_RULES` (behavioral inference from work history — does not require exact skill name in CV), `LANGUAGE_EVAL_RULES` (CV language or explicit mention). Use double-braces `{{}}` for literal JSON and single-brace for `.format()` injection.
...
4. **`analysis.py`** — Phase 3 alignment logic. `classify_skills(skills)` calls LLM via `tracked_llm_call(agent="analyzer")` with `SKILL_CLASSIFIER_PROMPT` and returns `dict[str, SkillType]`. `_core_skill(skill)` normalizes skill text before evaluation: strips parenthetical qualifiers and converts slash-separated alternatives to "or" format (e.g., "LangChain/LlamaIndex" → "LangChain or LlamaIndex"). `evaluate_skill_match(skill, context, skill_type)` selects rules via `_RULES_BY_TYPE` dict and calls LLM with `SKILL_EVAL_PROMPT`, returns a `SkillMatch` — decorated with `@traceable` for LangSmith. `analyze_alignment(job_profile, cv_text, chunks_stored, skill_types)` accepts an optional pre-computed `skill_types` to avoid redundant `classify_skills` calls — iterates required skills, retrieves context, evaluates each, returns `AlignmentAnalysis`. All LLM calls go through `tracked_llm_call` for Prometheus metrics.
